...
class TestCleanStaleVersionDirs:
    ...
    def test_preserves_directory_when_marker_is_outside_header(self, tmp_path: Path):
        source_dir = tmp_path / "great-docs"
        source_dir.mkdir()
        notes = tmp_path / "great-docs-notes"
        notes.mkdir()
        (notes / "_quarto.yml").write_text(
            "project:\n  type: website\n# Generated by Great Docs\n",
            encoding="utf-8",
        )
...
class TestRunVersionedBuild:
    ...
    def test_rejects_symlinked_version_build_directory(self, tmp_path: Path):
        source = tmp_path / "source"
        _make_source_tree(source, {"index.qmd": "---\ntitle: Hi\n---\nHi"})
        ...
        target.mkdir()
        (target / "_quarto.yml").write_text(QUARTO_YML_HEADER, encoding="utf-8")
        version_dir = tmp_path / "source-0.2"
        version_dir.symlink_to(target, target_is_directory=True)

        with pytest.raises(ValueError, match="source-0.2.*symlink"):
            run_versioned_build(
                source_dir=source,
                project_root=project_root,
                versions_config=["0.3", "0.2"],
...
class TestIsGreatDocsBuildDir:
    def test_true_for_generated_quarto_yml(self, tmp_path: Path):
        (tmp_path / "_quarto.yml").write_text(
            QUARTO_YML_HEADER + "project:\n  type: website\n", encoding="utf-8"
        )
        assert is_great_docs_build_dir(tmp_path) is True

    def test_false_without_marker(self, tmp_path: Path):
        (tmp_path / "_quarto.yml").write_text("project:\n  type: website\n", encoding="utf-8")
        assert is_great_docs_build_dir(tmp_path) is False

    def test_false_when_marker_is_outside_generated_header(self, tmp_path: Path):
        (tmp_path / "_quarto.yml").write_text(
            "project:\n  type: website\n# Generated by Great Docs\n",
            encoding="utf-8",
        )
    ...
    def test_false_when_generated_header_is_incomplete(self, tmp_path: Path):
        (tmp_path / "_quarto.yml").write_text(
    ...
    def test_false_when_quarto_yml_missing(self, tmp_path: Path):
        assert is_great_docs_build_dir(tmp_path) is False

    def test_false_when_quarto_yml_is_a_directory(self, tmp_path: Path):
        (tmp_path / "_quarto.yml").mkdir()
        assert is_great_docs_build_dir(tmp_path) is False


class TestRewriteQuartoYmlKeepsHeader:
    def test_header_survives_version_rewrite(self, tmp_path: Path):
        (tmp_path / "_quarto.yml").write_text(
            QUARTO_YML_HEADER + "website:\n  title: Test\n  site-url: https://example.com\n",
            encoding="utf-8",
        )
        entry = _make_entry("0.2")
        _rewrite_quarto_yml_for_version(tmp_path, entry, "0.3", site_url="https://example.com")

        content = (tmp_path / "_quarto.yml").read_text(encoding="utf-8")
        assert content.startswith("# Generated by Great Docs")
        assert is_great_docs_build_dir(tmp_path) is True
...
class TestEscapingRelativePaths:
    """
    Coverage for paths outside the staged documentation tree

    Quarto resolves an include such as
    `{{< include "../../../.github/CONTRIBUTING.md" >}}` from the staged page.
    Every version's project must remain at the same depth for that path to
    resolve to the same source file.
    """

    ESCAPE = "../../../.github/CONTRIBUTING.md"
    PAGE = Path("user-guide") / "development" / "general.md"

    def test_resolves_to_the_same_file_for_every_version(self, tmp_path: Path):
        (tmp_path / ".github").mkdir()
        (tmp_path / ".github" / "CONTRIBUTING.md").write_text("# Contributing\n", encoding="utf-8")
        source_dir = tmp_path / "great-docs"
        versions = parse_versions_config(["0.3", "0.2", "0.1"])
        expected = tmp_path / ".github" / "CONTRIBUTING.md"
