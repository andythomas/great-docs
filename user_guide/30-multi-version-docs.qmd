...
Versioned builds work in these directories:

- **`great-docs/`** stores the temporary Quarto project for the latest version and the assembled site in `great-docs/_site/`. Great Docs recreates the directory at the start of every build.
- **`great-docs-<tag>/`** stores one temporary, self-contained Quarto project for each historical version. Great Docs removes existing marked sibling directories before a versioned build and recreates only those selected for that build.
- **`.great-docs-cache/`** caches one API snapshot for each version that uses Strategy B (`git_ref`). Delete the directory to repeat API introspection during the next build.

Every version's Quarto project therefore sits one level below your project root, whether or not you configure `versions:`. A relative path in page content resolves to the same file either way.

A project configured with a single version, marked `latest: true`, creates no `great-docs-<tag>/` directories at all.

Add these to your `.gitignore`. Great Docs does this automatically when you run `great-docs init`, but if you set up versioning on an existing project you can add them manually:
...
The generated ignore block also retains `.great-docs-build/` for older build layouts.
`.great-docs/` stores snapshots created by `great-docs api-snapshot`.

Great Docs deletes only build directories whose `_quarto.yml` contains its generated marker. It preserves other `great-docs-*/` directories and prints a warning. If an unmarked directory would receive a configured version, the build stops and asks you to move or delete it.

## Next Steps

With versioned documentation configured, your readers can always find docs that match the release they're using. Add a `versions` list to your `great-docs.yml` and run `great-docs build` to get started.

- [Enable Multi-Version Documentation](../recipes/enable-multi-version-docs.qmd) is a hands-on recipe for setting up versioning
- [Use Version Fences and Badges](../recipes/use-version-fences-and-badges.qmd) covers annotating content with version-specific callouts
- [Deployment](deployment.qmd) covers publishing versioned sites to GitHub Pages
- [Configuration](configuration.qmd) covers all `great-docs.yml` options including version settings
