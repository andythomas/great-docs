...
## Previewing a PR or CI Build [version-badge new 0.15] {#pr-preview}
...
history). Yse an environment variable, a `.env` file, or `gh` instead.
:::
...
Have your workflow print the exact command after uploading the artifact so reviewers
discover it. Pin the command to the run that produced the build with
`--run ${{ github.run_id }}` (rather than `--pr`, which always resolves to the newest
build), so a reviewer reproduces exactly the build they're looking at:
...
```{.yaml filename=".github/workflows/docs.yml"}
      - name: How to preview this build locally
        ...
        run: |
          ...
          echo "  pipx run great-docs preview --run ${{ github.run_id }}"
```
:::
