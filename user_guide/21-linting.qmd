...
Great Docs recognizes these parser-independent directives:

- Cross-reference and exclusion: `%seealso` (see
  [Cross-Referencing](cross-referencing.qmd)) and `%nodoc` (see
  [Writing Docstrings](writing-docstrings.qmd#nodoc-exclude-an-item-from-documentation))
- Callouts: `%note`, `%warning`, `%caution`, `%danger`, `%important`, `%tip`, and `%hint`
- Version notices: `%versionadded`, `%versionchanged`, and `%deprecated`

Sphinx-style `.. name::` forms provide separate compatibility syntax. Callout and version
directives and numbered `.. [1]` citation markers work with every parser. Other RST
directives and inline cross-reference roles such as `` :py:exc:`ValueError` `` require
`parser: sphinx`.

Any other `%`-prefixed token, such as `%internal`, is flagged as unknown. If you see a false
positive, check for typos in the directive name.
